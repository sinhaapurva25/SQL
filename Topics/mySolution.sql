...
select * from albums where release_year=min(release_year);
...
select distinct b.name as 'Band_Name' from bands as b
...
where name = 'Master of Puppets' AND band_id = 2;

insert into bands(id,name) values (8,'The Local Train');
insert into albums(id,name,release_year,band_id) values(19,'Aalas Ka Pedh',2015,8);
...
albums.release_year 'Release Year',
...
count(albums.band_id) as 'Number of Songs'
from bands
join albums on bands.id = albums.band_id
join songs on albums.id = songs.album_id
