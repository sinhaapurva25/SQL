...
*/
select round(AA.k/BB.k,2)
from (select count(*) as k
	  from (select count(*) from request_accepted
			GROUP BY requester_id,accepter_id) A) AA, -- 4
	 (select count(*) as k
	  from (select COUNT(*) from friend_request
		    group by sender_id,send_to_id) B) BB; -- 5